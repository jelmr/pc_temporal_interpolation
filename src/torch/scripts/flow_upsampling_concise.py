...
import pathlib
import glob
sys.path.append("/home/jelmer/flownet")
...
if __name__ == "__main__":
    ...
    dpc = FileBackedDynamicPointCloud(args.in_hr_dir, args.in_hr_dpc)
    dpcd = FileBackedDynamicPointCloud(args.in_lr_dir, args.in_lr_dpc)
    dpco_dir = osp.join(args.out_dir, "up")
    dpco = FileBackedDynamicPointCloud(dpco_dir ,osp.join(dpco_dir, "frames.dpc"))
    dpco2_dir = osp.join(args.out_dir, "snap")
    dpco2 = FileBackedDynamicPointCloud(dpco2_dir ,osp.join(dpco2_dir, "frames.dpc"))

    upscale_dpc(dpc, dpcd, dpco, dpco_dir)
    snap(dpco, dpco2, dpco2_dir)
