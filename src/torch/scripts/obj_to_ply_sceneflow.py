...
BASE_DIR = "/home/jelmer/torch/src"
...
def convert(input_dir, obj_file_name_1, mtl_file_name_1, obj_file_name_2, mtl_file_name_2, ply_sf_file_name,ply_nm_file_name, n, nosf=False, write_ascii=True, u=None, v=None, random_indices=None):
    ...
    if u is None or v is None or random_indices is None:
        points_1, colors_1, normals_1, random_indices, u, v = sample(vertices_1, faces_1, textures_1, parts_1, n, input_dir, materials_1)
    else:
        points_1, colors_1, normals_1, random_indices, u, v = sample(vertices_1, faces_1, textures_1, parts_1, n, input_dir, materials_1, u=u, v=v, random_indices=random_indices)
    points_2, colors_2, normals_2, random_indices, u, v = sample(vertices_2, faces_2, textures_2, parts_2, n, input_dir, materials_2, random_indices=random_indices, u=u, v=v)
...
def convert_all(input_dir, output_dir, n, write_ascii=True, nosf=False):
    ...
    for (i, obj_file_1) in enumerate(files[:-factor]):
        obj_file_2 = files[i+factor]
        ...
        if osp.exists(mtl_file_1) and osp.exists(mtl_file_2):
            ...
            ply_sf_file = osp.join(output_dir, "sceneflow", f'{base_name}.ply')
            ply_nm_file = osp.join(output_dir, "normals", f'{base_name}.ply')
            if u is None or v is None or random_indices is None:
                random_indices, u, v = convert(input_dir, obj_file_1, mtl_file_1, obj_file_2, mtl_file_2, ply_sf_file, ply_nm_file, n, write_ascii=write_ascii, nosf=nosf)
            else:
                random_indices, u, v = convert(input_dir, obj_file_1, mtl_file_1, obj_file_2, mtl_file_2, ply_sf_file, ply_nm_file, n, write_ascii=write_ascii, u=u, v=v, random_indices=random_indices, nosf=nosf)
        else:
...
if __name__ == "__main__":
    ...
    pathlib.Path(osp.join(output_dir, "sceneflow")).mkdir(parents=True, exist_ok=True)
    pathlib.Path(osp.join(output_dir, "normals")).mkdir(parents=True, exist_ok=True)

    # Convert OBJ -> PLY
    convert_all(args.input_dir, output_dir, n=args.n, write_ascii=args.ascii, nosf=args.nosf)
