...
parser.add_argument('--log_dir', default='log_train_phase2', help='Log dir [default: log_train]')
...
TRAIN_DATASET = synthetic_dataset.SceneflowDataset(npoints=NUM_POINT)
TEST_DATASET = synthetic_dataset.SceneflowDataset(npoints=NUM_POINT, train=False)
...
def train():
    with tf.Graph().as_default():
        ...
        print(not_initialized_vars2)
        print("================")


        if len(not_initialized_vars2):
            sess.run(tf.variables_initializer(not_initialized_vars2))
...
def train_one_epoch(sess, ops, train_writer):
    ...
    for batch_idx in range(num_batches):
        ...
        if (batch_idx+1)%5 == 0 or True:
            log_string(' -- %03d / %03d --' % (batch_idx+1, num_batches))
            log_string('mean loss: %f' % (loss_sum / 1))
            loss_sum = 0
